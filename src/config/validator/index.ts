 ...
 * 聚合校验单 monitor 交易配置。
 *
 * 保持 monitor 配置校验与全局配置校验的既有顺序，
 * 以确保错误收集结果与日志语义不发生偏移。
 *
 * @param tradingConfig 单 monitor 交易配置
 * @param env 进程环境变量
 * @returns 聚合后的校验结果
 */
...
/**
 * 验证 Longbridge 认证启动配置与单 monitor 交易配置完整性。
 * @param options.env 进程环境变量
 * @param options.tradingConfig 单 monitor 交易配置
...
export async function validateAllConfig({
  ...
  if (autoSearchEnabled) {
    ...
    logger.info('做多席位标的: 自动寻标');
    logger.info('做空席位标的: 自动寻标');
  } else {
    logger.info(`做多席位标的: ${monitorConfig.longSymbol}`);
    logger.info(`做空席位标的: ${monitorConfig.shortSymbol}`);
  }
