...
/**
 * 通用任务类型（队列元素）。
 * 类型用途：买卖任务队列中的单项，携带 id、type、data（Signal）、createdAt；泛型 TType 为 BuyTaskType 或 SellTaskType。
 ...
 * 类型用途：表示立即买入任务，供 TaskQueue<BuyTaskType> 与 BuyProcessor 使用。
 * 数据来源：由信号流水线在入队时指定。
 * 使用范围：tradeTaskQueue、buyProcessor、mainProgram 等，仅内部使用。
 */
export type BuyTaskType = 'IMMEDIATE_BUY';
 ...
 * 类型用途：表示立即卖出任务，供 TaskQueue<SellTaskType> 与 SellProcessor 使用。
 * 数据来源：由信号流水线在入队时指定。
 * 使用范围：tradeTaskQueue、sellProcessor、mainProgram 等，仅内部使用。
 */
export type SellTaskType = 'IMMEDIATE_SELL';
