 ...
 * 类型用途：为公开的 MonitorTask 提供基础结构，统一 id 与去重键。
 * 使用范围：仅 monitorTaskQueue 类型定义内部使用。
 */
 ...
 * 类型用途：为公开的 MonitorTaskInput 提供基础结构，表达调用方入队时尚未生成 id 的任务形态。
 * 使用范围：仅 monitorTaskQueue 类型定义内部使用。
 */
...
/**
 * 监控任务（队列元素）。
 * 类型用途：监控任务队列中的单项，携带 id、type、dedupeKey 与 data；由 Processor 消费。
 * 数据来源：由 scheduleLatest 写入（id 由队列生成），MonitorTaskProcessor 出队消费。
 ...
 * 类型用途：调用方提交任务时传入的数据，不含由队列生成的 id。
