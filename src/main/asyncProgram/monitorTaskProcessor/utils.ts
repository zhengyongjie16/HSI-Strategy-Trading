...
import type { MonitorTaskContext, SeatSnapshot, StrategyRuntimeAndSeatReadiness } from './types.js';
import {
 ...
 * @param params.baseInstrumentSymbol 监控标的
 ...
 */
export async function evaluateStrategyRuntimeAndSeatReadiness(params: {
...
}): Promise<StrategyRuntimeAndSeatReadiness | null> {
  const { getContextOrSkip, refreshGate, baseInstrumentSymbol, longSnapshot, shortSnapshot } =
    params;
  const context = getContextOrSkip();
  if (!context) {
    logger.debug(`[MonitorTaskProcessor] 上下文缺失，跳过 monitor=${baseInstrumentSymbol}`);
    return null;
  }
  ...
  if (!snapshotValidity) {
    logger.debug(`[MonitorTaskProcessor] 席位快照失效，跳过 monitor=${baseInstrumentSymbol}`);
    return null;
  }
