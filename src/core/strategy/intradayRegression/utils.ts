import { isValidNumber, parseIndicatorPeriod } from './indicatorHelpers/utils.js';
...
import type { IndicatorState } from './indicatorHelpers/types.js';
...
export function needsDelayedVerification(config: SingleVerificationConfig): boolean {
  return config.delaySeconds > 0 && config.indicators.length > 0;
}
