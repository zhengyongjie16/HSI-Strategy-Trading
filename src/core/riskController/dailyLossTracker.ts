 ...
 * 当日亏损追踪器模块
 *
 * 功能/职责：按方向累计已实现亏损偏移；内部基于当日成交订单与过滤算法（filteringEngine）计算未平仓买入成本。
 * 执行流程：调用方通过 recalculateFromAllOrders 或 recordFilledOrder 传入/增量订单，通过 getLossOffset(direction) 获取当日亏损偏移；内部按 direction 维护订单与偏移。
 */
...
function createDirectionStateMap(): Record<'LONG' | 'SHORT', DailyLossState> {
  return {
...
/**
 * 创建当日亏损追踪器实例。
 * 按 direction 维护当日买入/卖出订单与亏损偏移，支持 resetAll、recalculateFromAllOrders、recordFilledOrder、getLossOffset。
...
export function createDailyLossTracker(deps: DailyLossTrackerDeps): DailyLossTracker {
  ...
  let statesByDirection = createDirectionStateMap();
  ...
  const lastAppliedProtectionBoundaryByDirection = new Map<'LONG' | 'SHORT', number>();
  ...
  function resetAll(now: Date): void {
    ...
    dayKey = nextKey;
    statesByDirection = createDirectionStateMap();
  ...
  /**
   * 启动时根据历史成交订单初始化当日状态。
   * protectionBoundaryByDirection 可选：按方向恢复保护性边界。
   */
  function initializeFromOrders(
    ...
    if (protectionBoundaryByDirection) {
      latestProtectionBoundaryByDirection.clear();
      lastAppliedProtectionBoundaryByDirection.clear();
      for (const [direction, boundaryMs] of protectionBoundaryByDirection) {
    ...
    const grouped: Record<'LONG' | 'SHORT', RawOrderFromAPI[]> = {
      LONG: [],
      SHORT: [],
    };
    for (const order of allOrders) {
      ...
      if (ownership?.baseInstrumentSymbol !== monitor.baseInstrumentSymbol) {
        continue;
      }
      ...
      grouped[ownership.direction].push(order);
    }
  ...
  function recalculateFromAllOrders(
    ...
    initializeFromOrders(allOrders, monitor, now, protectionBoundaryByDirection);
  }
  ...
  function recordFilledOrder(input: DailyLossFilledOrderInput): void {
    ...
    const protectionBoundary = latestProtectionBoundaryByDirection.get(input.direction);
    ...
    const currentState = statesByDirection[input.direction];
  ...
  function getLossOffset(direction: 'LONG' | 'SHORT'): number {
    return statesByDirection[direction].dailyLossOffset;
  }
