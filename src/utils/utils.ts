...
import type { StrategyRuntimeSeatSnapshot, StrategyRuntimeSnapshot } from './types.js';
 ...
 */
export function resolveStrategyRuntimeSeatSnapshot(
  _baseInstrumentSymbol: string,
  symbolRegistry: SymbolRegistry,
): StrategyRuntimeSeatSnapshot {
...
/**
 * 解析单个监控标的的运行时快照。
 * 默认行为：基于席位快照与 quotesMap 派生 StrategyRuntime 所需的行情与名称字段。
 *
 * @param baseInstrumentSymbol 监控标的代码
 * @param symbolRegistry 席位注册表
 * @param quotesMap 标的 -> 行情 Map
 * @returns StrategyRuntime 所需的运行时派生快照
 */
export function resolveStrategyRuntimeSnapshot(
  baseInstrumentSymbol: string,
  symbolRegistry: SymbolRegistry,
  quotesMap: ReadonlyMap<string, Quote | null>,
): StrategyRuntimeSnapshot {
  const seatSnapshot = resolveStrategyRuntimeSeatSnapshot(baseInstrumentSymbol, symbolRegistry);
  ...
  const monitorQuote = quotesMap.get(baseInstrumentSymbol) ?? null;
  return {
    ...seatSnapshot,
    longQuote,
    shortQuote,
    ...
    baseInstrumentName: monitorQuote?.name ?? baseInstrumentSymbol,
  };
}
