...
function buildDirectionCooldownKey(direction: 'LONG' | 'SHORT'): 'LONG' | 'SHORT' {
  return direction;
}

/**
 * 创建清仓冷却追踪器，记录保护性清仓成交时间并计算剩余冷却时长。
 * 内部以 direction 为键存储成交时间戳，查询时按冷却模式动态计算结束时间。
 * 与保护性清仓完成链路的协作方式：
...
export function createLiquidationCooldownTracker(
  ...
  const cooldownMap = new Map<'LONG' | 'SHORT', number>();
  const triggerCountMap = new Map<'LONG' | 'SHORT', number>();
  const { nowMs } = deps;

  /** 记录保护性清仓成交时间，无效时间戳不写入，避免脏数据影响冷却判断 */
  function recordCooldown({ direction, executedTimeMs }: RecordCooldownParams): void {
    // 无效时间戳不记录
    if (!Number.isFinite(executedTimeMs) || executedTimeMs <= 0) {
      return;
    }

    cooldownMap.set(buildDirectionCooldownKey(direction), executedTimeMs);
  }
  ...
  function restoreTriggerCount({ direction, count }: RestoreTriggerCountParams): void {
    if (!Number.isInteger(count) || count <= 0) {
      return;
    }

    triggerCountMap.set(buildDirectionCooldownKey(direction), count);
  }
  ...
  function clearMidnightEligible({ directionsToClear }: ClearMidnightEligibleParams): void {
    for (const direction of directionsToClear) {
      cooldownMap.delete(direction);
      triggerCountMap.delete(direction);
    }
  }
