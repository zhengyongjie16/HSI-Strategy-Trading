...
>
> 本次只更新本计划及对应设计合同，不修改生产代码、测试、配置或构建工具。“最终”指已确认的待执行文档，不表示重构已经实施。真实部署参数未读取、未猜测；生产配置迁移仍是发布阻断项。
...
| D5 | 构建全量校验，启动只校验选中策略 | 构建自动发现所有策略目录并验证入口、身份、配置和资产；应用启动只导入、读取和 prepare 选中策略。未选策略错误阻断正式全量构建，不因加载未选模块而阻断源码直接启动。新增策略仍需构建部署、重启，不支持热切换。 |
...
- 不增加多策略并发、热切换、运行期热发现、启动时导入全部策略、人工/生成式注册表、策略账本、策略持久化、跨进程 pending 恢复或生产 supervisor。构建期目录发现属于本次范围。
...
| 指标数学模块有真实 tools 消费者 | `tools/dailyIndicatorAnalysis/indicatorCalculators.ts:8–30`；`tsconfig.build.json` | 不能全部机械迁入私有目录，也不能把 tools 排除编译来掩盖残留。保留实际共享的中性数学模块，工具只依赖数学契约。 |
...
7. 依 D4/D5，原静态注册及启动全 registry 校验已经被取代；命名映射、按需动态加载、构建全量发现和结构测试须整体切换，不能只删注册文件。
...
src/main/businessEventProgram/emissionAdapter.ts
                                             普通decision白名单适配
src/services/indicators/runtime/              仅保留有实际复用的中性数学模块
```
...
优先保留原路径下已有真实工具消费者的 `ema.ts`、`kdj.ts`、`mfi.ts`、`rsi.ts` 及其必要中性 `utils.ts`/数学状态类型，避免无意义改路径。这些模块接收算法参数和调用方状态，不持有全局策略实例状态。

- 将 `services/indicators/profile/` 和 `runtime/index.ts` 的策略用途、固定参数选择、profile 校验与指标编排迁入具体策略。
- `adx.ts`、`macd.ts`、`psy.ts` 若最终搜索仍仅服务当前策略，则迁入私有 runtime；其依赖的 EMA 中性运算直接从保留源模块引用。
- 原共享 runtime/types 拆出 profile、真实运行态和具体 snapshot 等策略契约；只保留数学消费者真正需要的类型。策略内部直接使用私有 state 类型，删除旧公共 opaque handle、仅为宿主持有/回传服务的 Symbol 品牌、双重类型断言转换与 unwrap，不在私有目录复制该包装。
- `KDJIndicator` 等若同时供工具和数学使用，移到数学模块共同类型源，所有消费者直接 import，不经旧 `types/quote.ts` 转发。
- 具体 `IndicatorSnapshot`、用途列表、验证指标名解释、固定默认生产参数不得留在公共数学模块。
- `tools/dailyIndicatorAnalysis/indicatorCalculators.ts` 只跟进本次直接影响的 imports/types；不重构其全量计算、错误口径或 CLI。

验收不是要求 `services/indicators/` 目录完全消失，而是确认里面不再有本策略的政策或宿主可独立构造的策略运行态。
...
| 契约 | 必需内容 | 禁止内容 |
| --- | --- | --- |
| `StrategyDefinition` | `id: string`、模块相对 `configHref`、`prepare(rawText)`；入口固定命名导出 `strategyDefinition` | 单一实现字面量联合、中央注册、默认配置、具体定义自行扫描或装配其他策略 |
| `PreparedStrategy` | 捕获已校验只读配置的 `create(deps)` | 对外 config/profile、泛型擦除适配、宿主编译能力 |
| `TradingSignalStrategy` | strategyId；handleMarketEvent；invalidateDirection/All；resetForTradingDay；destroy | onVerified、外部 sample/pending/cache 管理端口、Broker 能力 |
...
- 跨平台约束：目录名必须保持实际大小写；构建拒绝大小写折叠后冲突的策略目录（如 `fooBar` 与 `foobar`），并拒绝 Windows 保留设备目录名（如 `con`、`nul`、`com1`）。不依赖当前操作系统恰好允许创建某个名字。
...
- 策略根从加载器自己的模块 URL 相对定位，不能来自 env/cwd。用目录枚举返回的真实名称逐段核对选中目录、入口、配置的精确拼写；检查真实路径边界，拒绝候选目录及入口/配置的符号链接或 junction，不以裸字符串前缀代替目录边界检查。
- 应用启动允许只枚举根目录名称来确认选中项实际拼写、与该项相关的大小写冲突；这不是全量策略校验，不读取未选目录内容、不导入未选模块。无关未选目录的格式/文件/配置错误由构建全量检查。
...
1. composition root 注入文件读取、目录元信息检查和动态模块加载能力；准备层不自行创建 SDK/业务资源。模块加载结果先接为 `unknown`，验证固定导出对象、id/configHref 字段和 prepare 函数，不用 `as StrategyDefinition`、any 或配置类型擦除绕过检查。
...
7. 这里只加载可信发布包内的本地程序，不是沙箱或第三方不可信插件平台。发布期间禁止原地并发替换运行中的策略文件；发布包一致性由停机部署和完整构建保障，不新增进程内热加载或文件锁平台。
8. 正式构建自动扫描所有策略并校验配置；运行时只校验选中项。未选策略损坏会阻断正式构建，但不通过启动全量导入而影响直接源码运行。构建和运行是不同阶段，不混算 prepare 次数。
...
1. 原文语法校验并取得 `unknown` JSON 值，不以断言绕过类型检查。
2. 用识别 JSON 字符串、转义、对象/数组层级的有界结构扫描检查重复 object key。每个对象各自维护 key Set；key 经 JSON 字符串解码后比较，覆盖 `"buy"` 与等价 Unicode 转义键。
3. `JSON.parse` 可以与扫描配合；即使先 parse，也必须仍从完整原文检查重复，不能在覆盖后的对象上检查。
...
监听并记录 observedAtMs
 ...
 → strategy.handleMarketEvent(context, emitter) 一次
 → 将返回的中性显示投影交 display owner
```

`allowNewEvaluation` 由原 ordinarySignalGuard、有效 currentDayKey、非开盘保护共同决定。不开新的宿主 delay policy，不为了显示第二次调用策略。监听时间和用于门禁判断的 clock 时间必须有效；不合法时在同步业务边界报告 fatal，不采样、不创建 origin 或输出。包围策略 handleMarketEvent 的同步调用边界也必须在返回/上抛前报告内部异常，不能只依赖外层 Promise.catch 稍后才关交易门禁。
...
emitter 可以被私有 pending 长期保存，handleMarketEvent 返回不使其失效。
...
2. 按 §4.5/§4.6 校验 ACTIVE_STRATEGY_ID、推导选中目录，确认入口精确路径后动态 import 一次；验证固定导出、ID、configHref。启动不全量导入或校验未选策略。
3. 只读取选中 configHref 原文一次，prepare 一次；未选模块零导入、零 JSON 读取、零 prepare/create。
4. 解析不含策略结构的全局交易/认证配置。以上失败都发生在 SDK 创建前。
5. 准备初始关闭的 gate、clock/scheduler、共享终止 owner 和 cleanup 计划；立即安装正常退出订阅，早于 selected prepared.create 和首个 SDK/业务资源创建。将旧 `waitForShutdownSignal` 的晚安装 Promise 接口改为注入同步回调的订阅接口，返回幂等释放函数；主流程仅等待共享终止 owner 的通知。
6. selected prepared.create 一次；实例返回立即登记唯一 strategy destroy cleanup，然后核验 strategyId/返回端口契约。
7. 逐个创建 SDK、行情、Trader、recorder、风险等宿主资源，返回后立即登记其 cleanup。
8. 使用完整资源和同一 strategy 引用创建 MonitorContext；context 不调用策略 factory。
9. 装配 queue、processors、事件/时间/lifecycle owners，注入共享终止授权。
10. 按原 startup snapshot/recovery/rebuild 流程恢复事实；成功且终态仍未关闭才放行。
11. prepared 不进入运行期 context/post-gate 依赖；运行期不再 prepare/create。
...
destroy 后 handleMarketEvent/invalidate/reset/迟到 callback 都先检查终态，零采样、零输出、零重排。reset 不能撤销 destroy。同步方向取消是事件驱动接线，不改成未来某轮轮询清理。
...
2. 实施 §4.5 严格 ID/目录双向映射、精确大小写、跨平台名字和路径边界；纯映射与文件 I/O 分开，供构建工具复用。
3. root 准备 helper 注入 env、文件/目录能力及动态 importer，而不是 definitions 数组；按 §4.6 只定位并导入选中入口，导出先以 unknown 校验，保留 import 原因。
4. 实施 §5.1 JSON/parser/profile 只读准备；读取/prepare 各一次，不增加 env fallback、配置 manifest 或生成清单。
...
**完成门禁**：动态加载和 prepare 无 SDK/网络/timer/create；未选模块零求值；不存在中央登记或 fallback；无 any、配置断言适配、重复类型别名。
...
1. 拆出真正共享数学与策略政策；同步更新数学工具 imports/type，不能反向依赖 core 私有目录。
...
**完成门禁**：策略独立测试覆盖 §10 的指标/时间/验证/生命周期行；外部只能经公共 port 操作；共享目录不残留策略政策。
...
9. 同步现有 ESLint/import boundary 规则和 architecture tests。只允许新增中性公共端口路径，不开 services → core 全目录例外。

**完成门禁**：无旧 owner、无 re-export/别名转发、无具体策略数据流经宿主；工具和测试能从真实源模块引用数学类型。

### WP7：构建与策略资产

**文件**：`package.json`；最小 `tools/strategyAssets/`（types/utils 按规范）及其 tests；`.env.example`、README、实际直接相关部署引用。

**步骤**：

1. 资产工具自动枚举源码策略根的每个一级子目录，复用 §4.5 命名/路径规则；按稳定顺序校验所有目录、大小写折叠冲突、必需文件，不能跳过缺 definition/config 的目录。没有任何策略目录也失败。不维护或生成 id/path 表。
2. 项目根从工具模块位置可靠定位；确认 `tsconfig.build.json` 当前 `rootDir=./`、`outDir=./dist`，生产入口为 `dist/src/index.js`，配置邻接路径为 `dist/src/core/strategy/.../config.json`。新增源码自动进入 tsc 的 src glob，不需要静态注册 import。
3. 一次构建：先完成目录清单元信息预检，然后 clean → tsc → 逐个确认编译 definition.js 存在并动态导入 → 校验其固定导出/id/指向自身相邻产物JSON的configHref → 将源config原文读取一次交该编译definition.prepare验证 → 将同一原文完整写入/原子替换到其产物configHref。不能验证后再次读取不同字节复制。编译定义的prepare是纯函数，目的JSON在复制前不存在也不得自行读取。全部成功才退出0，不 create、不访问SDK或认证配置。
4. source start 仍可为 `bun src/index.ts`；dist 单独验证。构建阶段源JSON到产物地址的复制是显式资产操作，不是运行时fallback；运行时任何一种入口都只读自身相邻地址。每次构建在独立进程中执行资产校验，不依赖上一轮模块缓存，也不以 cache-busting query 绕过缓存。
5. 开发循环不提供 watch：源码或策略资产变更后显式执行完整 build（按第 3 步 clean → tsc → 校验与同步），全部成功后才启动。不得继续运行过期 dist 或旧产物配置，也不得以 touch 假模块或源码路径 fallback 掩盖 JSON-only 未生效。
6. 缺入口的半成品策略目录、删除或非法的策略配置使构建非零退出，且不得将失败构建的产物当作通过验证的资产使用；运行中不热读配置，选择配置文件变化沿停机/重启生效。
7. 更新 scripts 与说明：仓库已删除 `dev:watch`/`build:watch`/`test:watch` 脚本及 `concurrently` 依赖，不得重新引入会运行过期 dist 的编排；README 与实际命令一致，不额外移除无关依赖。

**完成门禁**：JSON-only 修改在重新 build 并重启后确实更换启动读取值；删除/非法策略配置使构建非零退出且不写出对应产物资产；空 cwd clean-dist 启动可加载相邻资产。工具测试使用离线替身，禁止为构建/资产测试连接真实交易账户。

真实 production config.json 未提供时，正式 build/发布仍阻塞；资产工具测试在临时 fixture 工程/策略目录树中验证，不能把测试配置写入生产策略资产假装完成。新增/删除整个目录及未选策略配置非法也必须进入构建测试。
...
4. 停旧进程并等待安全收口；部署完整程序、策略目录、ACTIVE_STRATEGY_ID、JSON 和构建资产。新增策略只新增目录并设置选择值，无需修改中心文件，但仍必须全量构建通过后重启。
...
| 数学 ema/kdj/mfi/rsi 等 | 真实中性复用保留，策略专用部分迁移 | dailyIndicatorAnalysis、数学测试、共享 types |
...
| types/quote 的具体 snapshot | 迁私有；共享数学结构另有中性源 | marketMonitor、tools KDJ 类型、riskCheckContext |
...
| ID | 建议归属 | 场景与必须断言 |
| --- | --- | --- |
| T01 | `tests/app/startup/prepareStrategy.test.ts` | 非法/缺失/非精确ID、选中目录/入口缺失、身份或configHref错误、旧键presence均在SDK前失败；未选definition零import/read/prepare/create。未选模块或JSON损坏不影响选中项启动，但全量build必须失败。 |
| T02 | `tests/core/strategy/intradayRegression/config.test.ts` | 缺字段、null、未知字段、重复键/转义等价键、非法类型/DSL/指标失败；对象/数组深层冻结。 |
...
| T13 | 同上/策略工厂 | callback先终态化；取消后同key新entry不被旧callback删除；destroy后旧timer、handle/reset均零副作用。 |
...
| T29 | `tests/architecture/strategyBoundary.test.ts` | 生产宿主仅由prepareStrategy动态加载选中入口；公共类型/数学无私有策略依赖，策略之间无私有import/聚合barrel；旧owner及静态/生成注册表不存在；宿主无IndicatorIncrementalRuntime，私有实现无旧opaque品牌转换/unwrap包装链。 |
| T30 | architecture + app integration | 临时策略根新增不同schema/私有指标目录，不改任何中心文件即可选择运行；只import选中模块且只create一次；构造/销毁身份正确。不永久增加第二套生产策略。 |
| T31 | `tests/tools/strategyAssets/` | 自动枚举全部一级目录，缺definition/config、名字冲突、未选坏配置均使build失败；编译definition.prepare无create，验证与复制同一JSON原文；dist/src相邻资产、空cwd、移走源码仍加载dist，无fallback。 |
| T32 | 离线重建集成 | JSON-only及整策略目录新增/删除后重新build均更新产物资产；非法/缺入口/失败时构建非零退出且不写出该策略的产物资产；新一轮definition校验无旧模块缓存；非根cwd、空dist首次build成功。 |
| T33 | `tests/core/strategy/utils.test.ts`、startup/资产测试 | 严格ID双向映射；拒绝foo-1、连续/尾随连字符、路径字符、设备名、目录大小写错误/折叠冲突、symlink/junction偏离目标。无需在大小写不敏感FS上强造冲突目录，可对枚举元信息fixture测全量校验。 |
...
rg -n "services/indicators|indicatorHelpers|KDJIndicator|formatSignalConfig" tools/dailyIndicatorAnalysis src tests mock
rg -n "concurrently|dev:watch|build:watch|test:watch|--watch" package.json README.md
```

逐项分类：

- 具体 snapshot/profile 名称只允许在策略私有实现及直接测试，真实中性数学类型单独判定；不能一刀切字符串删除。
...
补 bounded import graph/ESLint 测试，检查运行时代码和 `import type`、静态导出及字面量/计算动态 import；只匹配一条 import 字符串不足。`app/startup/prepareStrategy.ts` 是唯一生产宿主动态装配边界，其 importer 由 composition root 注入；相关模块范围内不得在其他宿主位置另开任意策略加载路径。构建工具可按同一校验合同逐个加载编译definition，但不create；测试fixture作为明确范围处理。受准加载边界单独检查URL构造与导入前路径验证。禁止策略间私有imports/聚合barrel、中央静态/生成清单；结合实际选中模块求值计数验证，不能用静态图无边断言动态隔离已经通过。
...
- [ ] D1–D5 已落实，未新增未获授权的交易政策；新增策略只新增目录，无中央人工/生成登记，构建全量校验与启动选中校验均有证据。
...
**当前状态：** D1–D5 已由用户确认，免注册自动加载已纳入最终执行计划及对应设计合同；重点调用链和命名/构建边界已复核。此前旧相关测试54项通过，不代表目标实现通过。本文件是最终待执行文档：生产代码、JSON、动态加载器、宿主接线、终态队列、构建与资产工具尚未实施；开发 watch 已按用户决定从仓库移除（含 `concurrently` 依赖），不再作为交付目标；正式构建/发布仍需真实配置与全部目标验收证据。
