...
基于 Longbridge OpenAPI SDK for Node.js / bun / TypeScript 的恒生指数趋势延续策略专项程序。
...
当需要你给出修改或重构方案以及落实方案时必须符合以下规范:

- 不允许给出兼容性或补丁性的方案
- 不允许过度设计，保持最短路径实现且不能违反第一条要求
- 不允许自行给出我提供的需求以外的方案，例如一些兜底和降级方案，这可能导致业务逻辑偏移问题
- 方案给出的测试用例必须是高价值的测试，例如高价值的回归测试或者整体链路测试，不要给出冗余的，细碎的或低质量的测试
